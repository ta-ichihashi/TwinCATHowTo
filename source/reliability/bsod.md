...
まずは準備として下記の通りNTFSでフォーマットされたUSB接続の外部ストレージドライブ（固定ディスクタイプ）をIPCのUSBポートへ接続してください。（ {numref}`external_storage` ）

```{warning}
* USBメモリなどのリムーバブルドライブは使用できません。固定ディスクとして認識する外付けHDD, SSDをご選択ください。
...
![](image/crash_control_new_string.png){align=center}
`````
`````{grid-item} 
2. エントリー名を `DedicatedDumpFile` に設定
![](image/new_dedicated_dump_file.png){align=center}
`````
`````{grid-item} 
3. ダブルクリックして値を保存先に設定する。
例えば、Dドライブが外付けUSBストレージでその直下に ``dedicateddumpfile.sys`` という名前で仮想メモリファイルを作る場合は右図の通り設定する
`````
`````{grid-item} 
![](image/path_dedicated_dump_file.png){align=center}
`````
``````
...
`````
`````{grid-item} 
![](image/start-search.png){align=center}
`````
`````{grid-item} 
2. ``SystemPropertiesAdvanced`` と入力してEnterキーを押します。
`````
`````{grid-item} 
![](image/systempropertiesadvanced.png){align=center}
`````
`````{grid-item} 
3. ``Advanced`` タブが開いている事を確認し、 ``Startup and Recovery`` 内の ``Settings...`` ボタンをクリックします。
`````
`````{grid-item} 
![](image/startup_and_recovery.png){align=center}
`````
`````{grid-item} 
4. 次の3点を変更します。
    ```{csv-table}
...
`````
`````{grid-item} 
![](image/complete_memory_dump.png){align=center}
`````
``````
...
```{admonition} NotMyFaultツール

* [メモリ ダンプ ファイルを手動で生成する](https://learn.microsoft.com/ja-JP/troubleshoot/windows-client/performance/generate-a-kernel-or-complete-crash-dump#manually-generate-a-memory-dump-file)

* [NotMyFaultツール](https://download.sysinternals.com/files/NotMyFault.zip)
```
...
1. 下記サイトにある Windows SDK からインストールする方法で WinDbg をインストールします。
   ...
   [WinDbgのダウンロード](https://learn.microsoft.com/ja-jp/windows-hardware/drivers/debugger/debugger-download-tools)
