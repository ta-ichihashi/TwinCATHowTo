...
ここに記載されているとおり、bit型のデータが1バイトづつ間隔を空けてADSメモリにマッピングされ、Modbusアドレス `0x0000` ～ `0x7fff` に配置されています。たとえば、次のEL2008の各アドレスは、%QX89.0 ～ %QX89.7 に配置されていることがわかります。ここから、Modbusアドレス上では、出力コイルの、 0x0000 $+$ 8 $\times$ 89 $+$ 0 $=$ 712 を先頭に、712～720 （16進数では 0x02C8 ～ 0x02D0）のアドレスで Modbus クライアント側はアクセスすることができます。

![](assets/2025-11-26-14-48-39.png){align=center}

続いて`<InputRegisters>` や`<OutputRegisters>` セクションは WORDデバイス（16bit整数サイズ）の入出力データエリアです。こちらについても同様の方法で、EtherCATやEtherNet/IPなどの入出力データを直接モニタ、書き込む事ができます。
