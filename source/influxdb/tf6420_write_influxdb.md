...
```{note}
チャンクサイズの自動調整機能は次図のとおり働きます。上のグラフは、黄色がDBへ書き込む目標アドレスで、緑色がPLCサイクル毎に記録されている実際のバッファアドレスです。緑色が黄色に交わった時点で階段一段分のデータのDBへの書き込みコマンドが発行されます。黄色の目標アドレスは、キューの使用サイズによりダイナミックに変化していることがわかります。

通常はminimum_chunk_sizeを下限としたサイズのチャンクで小刻みにDBに書込みを行いますが、別クライアントからDBに対して大量のデータ問い合わせが発生したり他のプロセスにより一時的にリソース不足に陥った場合、一時的にバッファの使用量が増えます。この際、自動的にチャンクサイズを大きくすることで、DBのスループットが回復した際の書込み回数が少なくなり、DB書込みのオーバヘッドが削減されるため、速やかにバッファ容量を回復させることができます。
