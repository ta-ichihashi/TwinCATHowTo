...
一つのデータグラムには、 1:コマンド種別、2:論理アドレス（論理書き込み: LWR, 論理読み込み:LRD, 論理読み書き:LRW）、3:データグラム長、4:送受信データ、で構成されたコマンドが格納されています。さらにこのコマンドは、「Sync unit」という単位で複数組み合わされており、最後にワーキングカウンタというデータが付加されます。

![](assets/datagram_structure.png)

ひとつのデータグラムには複数のコマンドで構成されていることから、複数のサブデバイスをまたいでデータの読み書きを処理する必要があります。このサブデバイスをまたぐたびに正常に処理されたことを確認してカウントアップするカウンタが、 **ワーキングカウンタ** です。

```{admonition} 参考
{ref}`section_whatis_sync_unit`
```

一つのデータグラムにおいてワーキングカウンタが繰り上げられるイメージは次のとおりです。メインデバイスは、Sync unitごとに、かつ、上限の1498Byteを超えない単位で複数のコマンドを構成したデータグラムを計画します。コマンドに含まれる論理アドレスは必ずしも単一のサブデバイス内にあるとは限りません。複数のサブデバイスをまたいでFMMUを通じたデータ入出力が行われます。図の例では緑色のWKC=*で示された対象のターミナルがこれに該当します。

メインデバイスは各サブデバイスで処理されたフレームがメインデバイスに戻った際に、このワーキングカウンタをチェックします。計画とおり4という値であれば正常にデータが処理されたものとみなされ、入力データであればこの値をPLCやモーションなどの値に反映します。しかし、値が適切でなければこのデータは破棄されます。
