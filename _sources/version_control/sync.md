...
```{warning} 
Pull時に競合が発生すると、自動マージされず、一旦はリポジトリの状態に更新されます。
```
...
1, Merge, `TcProjectCompare` を出現させ、競合個所のある個所について1行づつどちらを採用するか選んで解決することができます。となりにある`Compare Files`ボタンは差分表示のみを行います。左側が前回Pushした時点から自分の行った変更、右側がリモートリポジトリで行われた変更が一覧されます。行単位マージの詳細方法は次の節をご覧ください。
...
{ref}`figure_conficts` 内の、`Merge`ボタンを押すことで、次の通り競合個所の差分が一覧されます。
...
自分の行った変更が左側に反映されると背景が橙色に変わり、左右が同一の内容になります。
...
プロジェクトに戻りますので、マージした結果が反映された確認します。テスト環境やIPCへ適用し、動作確認まで行ってください。

テストが完了して、マージした内容に問題がないようでしたら、再度Team ExplorerのSyncメニューに進み、次図の通り `Continue` を押します。最初からやりなおす場合は、 `Abort` を押します。`Abort`を押すと、Pullを実施する前（自分の変更が反映された状態）に戻ります。

![](assets/2024-02-20-17-10-56.png){align=center}

全てのファイルでマージが終了しましたら、Pullは完了です。続けてPushを行ってください。